 ...
 * 脚本名称：大玩家
 * 活动规则：每日签到，30天60币
 * 脚本说明：添加重写进入"大玩JHJ店"小程序-我的-会员信息，即可获取 Token，支持多账号，兼容🐉青龙。
 * 环境变量：dawanjia_data=[{"Phone": "抓包响应体phone","userId": "抓包响应体userId"}]
 * 更新时间：2024年8月8日 16点45分
 * 图标地址：https://mirror.ghproxy.com/https://raw.githubusercontent.com/zyhwjl2022/QuantumultX/master/zyhwjl/dawanjia/icon/dawanjia.png

--------------- Quantumult X 配置 ---------------

[MITM]
hostname = capi.lzyun.vip

[rewrite_local]
https://capi.lzyun.vip/leaguers/info url script-response-body https://raw.githubusercontent.com/zyhwjl2022/QuantumultX/master/zyhwjl/dawanjia/dawanjia.js

[task_local]
30 8 * * * https://raw.githubusercontent.com/zyhwjl2022/QuantumultX/master/zyhwjl/dawanjia/dawanjia.js, tag=玩美世界, img-url=https://mirror.ghproxy.com/https://raw.githubusercontent.com/zyhwjl2022/QuantumultX/master/zyhwjl/dawanjia/icon/dawanjia.png, enabled=true

 */
const $ = new Env('大玩家');
$.is_debug = getEnv('is_debug') || 'false';  // 调试模式
$.userInfo = getEnv('dawanjia_data') || '';  // Token
    ...
    if ($.userArr.length) {
        ...
        for (let i = 0; i < $.userArr.length; i++) {
            ...
            $.headers = {
                ...
                'User-Agent': 'Mozilla/5.0 (iPhone; CPU iPhone OS 18_0 like Mac OS X) AppleWebKit/605.1.15 (KHTML, like Gecko) Mobile/15E148 MicroMessenger/8.0.49(0x18003137) NetType/WIFI Language/zh_CN',
                'Referer': 'https://servicewechat.com/wx8fea8c067a70e66c/14/page-frame.html'
            }
            // 签到
            await signin();
            await $.wait(1000);
...
async function signin() {
    ...
    let opt = {
        ...
        body: $.toStr({
            'SignInID': 'ae1c3dcd-d9c7-4ff5-9d60-8c7bf4d0c4fd',
            'Longitude': 113.76666232638888,
            'Latitude': 34.770054796006946,
        })
    };

    // 发起请求
    var result = await Request(opt);
    if (result?.code == 0 && result?.success && result?.success == true) {
        msg += `✅ ${$.Phone}签到成功\n`;
    } else if (result?.code == 0 || result?.success == false) {
        msg += `⛔️ ${$.Phone}签到失败:${result?.msg}\n`;
    } else {
        msg += `⛔️ ${$.Phone}签到错误\n`;
        $.log($.toStr(result));
    }
...
async function signinDays() {
    ...
    // 构造请求
    let opt = {
        url: `https://capi.lzyun.vip/leaguers/signin/progress`,
        headers: $.headers,
        params: {
        }
    };
    ...
    // 发起请求
    var result = await Request(opt);
    if (result?.code == 0 && result?.data && result?.data?.CurrentDay) {
        msg += `✅ ${$.Phone}已签到：${result?.data?.CurrentDay}天\n`;
    } else if (result?.code == 0) {
        msg += `⛔️ ${$.Phone}获取签到天数失败:${result?.msg}\n`;
    } else {
        msg += `⛔️ ${$.Phone}获取签到天数错误\n`;
        $.log($.toStr(result));
    }
    $.messages.push(msg.trimEnd()), $.log(msg.trimEnd());
}
...
function GetCookie() {
    try {
        ...
        const token = header['ch-token'];
        const body = $.toObj($response.body);
        const Phone = body?.data?.Phone;
        if (token && Phone) {
            const user = $.userArr.find(user => user.Phone === Phone);
            if (user) {
                msg += `更新用户 [${Phone}] Token: ${token}\n`;
                user.token = token;
                user.Phone = Phone;
            } else {
                msg += `新增用户 [${Phone}] Token: ${token}\n`;
                $.userArr.push({ "Phone": Phone, "token": token });
            }
            // 写入数据持久化
            $.setdata($.toStr($.userArr), 'dawanjia_data');
            $.messages.push(msg.trimEnd()), $.log(msg.trimEnd());
        }
    } catch (e) {
