...
[Script]
获取捷停车userId = type=http-request, pattern=^https:\/\/sytgate\.jslife\.com\.cn\/core-gateway\/order\/carno\/pay\/info, requires-body=1, max-size=0, script-path=https://raw.githubusercontent.com/zyhwjl2022/QuantumultX/master/zyhwjl/jparking/jparking_sign.js

捷停车签到 = type=cron, cronexp=15 9 * * *, timeout=60, script-path=https://raw.githubusercontent.com/zyhwjl2022/QuantumultX/master/zyhwjl/jparking/jparking_sign.js, script-update-interval=0
...
[rewrite_local]
^https:\/\/sytgate\.jslife\.com\.cn\/core-gateway\/order\/carno\/pay\/info url script-request-body https://raw.githubusercontent.com/zyhwjl2022/QuantumultX/master/zyhwjl/jparking/jparking_sign.js

[task_local]
15 9 * * * https://raw.githubusercontent.com/zyhwjl2022/QuantumultX/master/zyhwjl/jparking/jparking_sign.js, tag=捷停车签到, enabled=true

================ Loon 配置 ================
[MITM]
hostname = sytgate.jslife.com.cn

cron "15 9 * * *" script-path=https://raw.githubusercontent.com/zyhwjl2022/QuantumultX/master/zyhwjl/jparking/jparking_sign.js, tag=捷停车签到

http-request ^https:\/\/sytgate\.jslife\.com\.cn\/core-gateway\/order\/carno\/pay\/info script-path=https://raw.githubusercontent.com/zyhwjl2022/QuantumultX/master/zyhwjl/jparking/jparking_sign.js, requires-body=true, timeout=10, enabled=false, tag=获取捷停车userId
...
async function main() {
  for (let i = 0; i < userIdArr.length; i++) {
    ...
    $.taskMap = { "T00": "签到", "T01": "浏览", "T02": "看视频", "T45": "分享", "T46": "邀请" };
    ...
    // 领取分享奖励
    $.taskMap['T45'] && await receive("T45");
    
    // 领取邀请任务
    await invite();

    // 领取邀请奖励
    $.taskMap['T46'] && await receive("T46");
...
async function share() {
  let result = await httpRequest(options(Api.complete.url, `{"userId":"${$.userId}","reqSource":"APP_JTC","taskNo":"T45","token":"${$.token}"}`));
  debug(result, "share");
  if (!result.success) {
    console.log(`❌ 领取${$.taskMap['T45']}任务失败: ${result.message}`);
    delete $.taskMap['T45'];
  }
}
...
async function invite() {
  let result = await httpRequest(options(Api.complete.url, `{"userId":"${$.userId}","reqSource":"APP_JTC","taskNo":"T46","token":"${$.token}"}`));
  debug(result, "invite");
  if (!result.success) {
    console.log(`❌ 领取${$.taskMap['T46']}任务失败: ${result.message}`);
    delete $.taskMap['T46'];
  }
}
